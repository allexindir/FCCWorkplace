...
def plot_efficiency(df,mode_names,label):
    ...
    # for cur_mode in mode_names:
    #     print(f"{cur_mode} has {len(set(eff[cur_mode]))} unique values of efficiency.")
...
# def plot_cutflow():
#     # We want to display for each mode the number of events left over without cuts, after cuts, then after BDT selection
#     #This requires us to have: The expected number of events, the raw number of events with uproot, 
#     # Turn off ROOT graphics/warnings to keep the terminal clean
...
#     # Loop over each directory inside the batch folder
#     for process_name in sorted(os.listdir(base_dir)):
#         process_path = os.path.join(base_dir, process_name)
        
#         # Skip if it's a file instead of a directory
#         if not os.path.isdir(process_path):
#             continue
            
#         # look for tables called "events"
#         chain = ROOT.TChain("events")
        
#         file_count = 0
#         # Walk through the directory to find all .root files (handles chunks/subfolders)
#         for root, dirs, files in os.walk(process_path):
#             for file in files:
#                 if file.endswith(".root"):
#                     chain.Add(os.path.join(root, file))
#                     file_count += 1
                    
#         # Get total entries across all chained files
#         total_events = chain.GetEntries()
        
#         print(f"{process_name:<45} | {total_events:<12,} | {file_count:<5}")

#         #after getting the total events in the root files, we need the events after the cuts
